...
const pegarPost = async () => {
  const mensage = [];
  const order = Rl(_h(db, "posts"), Cl("date", "desc"));
  const snapShot = await ff(order);
...
const editarPosts = async (postId, text) => {
  const atualizarPostsEditados = mh(db, "posts", postId);
  await mf(atualizarPostsEditados, {
    text
  });
};
const deletarPost = async (postId) => {
  gf(mh(db, "posts", postId));
};
...
const postar = "/assets/checked.144db942.png";
          ...
          <div class="position-username">
            ...
            <p class="post-date">${posts.date}</p>
              ...
              <button id="${posts.id}salvar" class="btn-postar editado">
                <img class='editar-img' src=${postar} alt='salvar'>
              </button>
